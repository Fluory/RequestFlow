...
- `verify:full` – `verify` + Playwright smoke flow (upload → worker with an AI stub → review → approve; needs `playwright install chromium` or `PW_CHROMIUM_PATH`); the full AI eval run joins with #29/#30. `pnpm verify:full` – before a release, after risky refactors or with PR label `verify-full`.
