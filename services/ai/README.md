...
- **Scanned cases** are image-only PDFs without a text layer. OCR exists (#23) but is off by default
  (`AI_PDF_OCR=off`) and needs models that CI does not download, so the replay runs them without OCR:
  the service returns `no_text` without a model call and the expectation is "all fields missing"; the
  real values are kept under `after_ocr` in `expected.json`. **Known gap:** these three cases therefore
  test the no-text path, not OCR quality, and they add trivially correct "missing" observations to the
  missing precision/recall of header fields. Running them with OCR (models in CI, recorded responses,
  `after_ocr` as expectation) is a follow-up.
